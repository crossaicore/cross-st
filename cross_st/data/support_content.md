...
> Auto-generated by `script/build_ask_corpus.py`. Do not hand-edit; edit the source files (`docs/wiki/*.md`, `CHANGELOG.md`, `cross_st/data/support_faq.md`, and CLI argument declarations) and re-run the build script.
...
- `--dry-run`: Show what would be changed without calling any AI or writing to the file
- `--include-fixed`: Include reports previously generated by st-fix in auto-selection. Default: skip them so the originals are fixed instead of "fixing the fix".
...
Generated from the current CLI argument declarations. These establish option names; the wiki explains workflows. Dynamic values depend on configuration. Changelog entries describe historical releases, not necessarily current behavior.

### st

Source: `cross_st/st.py`.

- `input_file`: Path to the .json or .prompt file
- `--site`: Define Discourse site to use, default is [configured value]
- `-a`, `--agent`: Agent to start with, default is [configured value]
- `-b`, `--bang`: Submit prompt to each AI in parallel (bang!), default: no bang
- `-q`, `--quiet`: Minimal output, default: quiet on
- `-v`, `--verbose`: Verbose output, default: verbose off

### st-admin

Source: `cross_st/st-admin.py`.

- `--version`: Print the installed cross-st version and exit
- `--setup`: Interactive first-run wizard: checks environment, collects API keys, writes ~/.crossenv
- `--discourse-setup`: (Re-)run the crossai.dev community onboarding wizard independently of --setup
- `--discourse`: Manage Discourse site connection and default posting category
- `--check-tos`: Check whether the stored T&C version is current; prompt re-acceptance if stale
- `--ask-telemetry`: Enable or disable anonymous st-ask usage telemetry (default off)
- `--show`: Print all current settings and exit
- `--get-default-ai`: Print the current default AI provider and exit
- `--set-default-ai`: Set the default AI provider (e.g. anthropic, xai, gemini)
- `--set-ai-model`: Set a per-provider model override (e.g. xai=grok-3)
- `--add-agent`: Add or update an AI agent in ~/.cross_ai_models.json (e.g. anthropic-opus=anthropic:claude-opus-4-5; omit :MODEL for handler default)
- `--remove-agent`: Remove a user-defined agent from ~/.cross_ai_models.json
- `--list-agents`: Print the agent registry (one row per loaded agent)
- `--check-keys`: Live-test configured AI provider keys (may use provider quota)
- `--set-tts-voice`: Set the TTS voice string (written to TTS_VOICE in .env)
- `--set-template`: Set the default prompt template name
- `--set-editor`: Set the editor (written to EDITOR in .env)
- `--init-templates`: Seed ~/.cross_templates/ with bundled default .prompt files
- `--overwrite-templates`: With --init-templates: replace files that already exist
- `--upgrade`: Upgrade cross-st from PyPI (pipx or pip) and macOS Homebrew platform tools
- `--cache-info`: Print cache path, file count, and total size
- `--cache-clear`: Delete all cached AI responses
- `--cache-cull`: Delete cache entries not accessed in the last DAYS days

### st-analyze

Source: `cross_st/st-analyze.py`.

- `json_file`: Path to the JSON file
- `--agent`: Define AI to use, default is [configured value]
- `--cache`: Enable API cache, default: enabled
- `--no-cache`: Disable API cache
- `--site`: Select Discourse site to use, default is [configured value]
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output
- `--ai-title`: Generate a title (max 10 words) → stdout
- `--ai-short`: Generate a short summary (max 80 words) → stdout
- `--ai-caption`: Generate a caption (100–160 words) → stdout
- `--ai-summary`: Generate a summary (120–200 words) → stdout
- `--ai-story`: Generate a comprehensive story (800–1200 words) → stdout

### st-bang

Source: `cross_st/st-bang.py`.

- `prompt`: Path to the prompt file (e.g. subject.prompt)
- `--agent`: AI to use for the optional merge step (default: [configured value])
- `--cache`: Enable API cache (default: enabled)
- `--no-cache`: Disable API cache
- `-m`, `--merge`: Merge all stories into a master story via st-merge.
- `-s`, `--st`: Load the final .json container into the st app.
- `--force`: Regenerate all stories even if they already exist in the container.
- `-v`, `--verbose`: Enable verbose output.
- `-q`, `--quiet`: Suppress progress table (minimal output).
- `--timeout`: Per-job timeout in seconds (default: 600 = 10 min). 0 = no timeout.

### st-cat

Source: `cross_st/st-cat.py`.

- `json_file`: Path to the JSON file
- `-s`, `--story`: Story number (integer), default: 1
- `-f`, `--fact`: Fact-check report number (integer), default: none
- `--markdown`: Story markdown, default: off
- `--hashtags`: Story hashtags, default: off
- `--spoken`: Story spoken text, default: off
- `--text`: Story text body, default: off
- `-t`, `--title`: Story title, default: off
- `--prompt`: Print the original prompt text stored in the container (default if no other field specified)
- `-v`, `--verbose`: Verbose output, default: off
- `-q`, `--quiet`: Enable minimal output

### st-cross

Source: `cross_st/st-cross.py`.

- `json_file`: Path to the JSON container file
- `--cache`: Enable API cache (default: enabled)
- `--no-cache`: Disable API cache (no read, no write). NOTE: does not by itself force a re-fact-check — see --force.
- `--force`: Bypass resume detection AND clear all existing fact[] entries before launching, then re-run every cell. Use this when you've changed the fact-check prompt or want fresh verdicts. Implies --no-cache so the new payloads don't pull stale cached responses.
- `--timeout`: Per-cell wall-clock timeout in seconds (default: 1800 = 30 min). 0 = no timeout. Applies to the whole st-fact subprocess for one (story, fact-checker) pair; must be large enough to cover all segments plus any retries (see --retry-budget).
- `--skip-gen`: Skip Step 1 (story generation). Normally auto-detected: if all AI stories already exist in the container, Step 1 is skipped automatically without this flag.
- `-v`, `--verbose`: Enable verbose output. Implies no live table in Step 2 — per-cell 'Generating fact-check: …' lines would otherwise interleave with the live redraw. Step 1 (generation) still shows its live table.
- `-q`, `--quiet`: Suppress live table (minimal output).
- `--dry-run`: Preview the Step 2 matrix without running anything. Shows pending vs already-complete cells, a per-row and per-column summary, and exits. Useful before committing 30+ minutes to a full N×N run. Implies --skip-gen.
- `-p`, `--parallel`: Run cells in parallel, gated by per-provider rate-limit semaphores (default).
- `--sequential`: Run cells one at a time. Useful for debugging or very low-quota accounts.
- `--max-concurrency`: Override the per-provider concurrency cap. Default: uses cross_ai_core.get_rate_limit_concurrency() per make (xai=3, anthropic=2, openai=3, perplexity=2, gemini=5).
- `--retry-budget`: Per-segment retry budget passed through to st-fact (default: 45 = one ~15+30 s backoff cycle). 0 = unlimited; matches pre-PAR-1 behaviour. Independent of --timeout: --retry-budget caps how long one API call spends on transient-error retries, while --timeout caps the entire cell's wall clock.

### st-domain

Source: `cross_st/st-domain.py`.

- `--name`: Domain slug (snake_case, e.g. supply_chain)
- `--dir`: Output directory  (default: [configured value])
- `--agent`: AI provider for suggestions + smoke-test  (default: [configured value])
- `--n-claims`: Claims per domain  (default: [configured value])
- `--no-smoketest`: Skip Phase 4 validation smoke-test
- `--set`: Benchmark set config to register the new domain in after saving
- `--cache`: Enable AI response cache (default: on)
- `--no-cache`: Disable AI response cache
- `-v`, `--verbose`: Consult command help for details.

### st-edit

Source: `cross_st/st-edit.py`.

- `json_file`: Path to the JSON file
- `-s`, `--story`: Story data selected (integer), default 1
- `-f`, `--fact`: Fact-check to edit (integer), default None
- `-spoken`, `--spoken`: Edit the text to speech source text default: do not edit
- `-md`, `--markdown`: Edit the markdown data, default: do not edit
- `-title`, `--title`: Edit the title data, default: do not edit
- `-text`, `--text`: Edit the text data, default: do not edit
- `--view`: View with Glow and edit with Vi, default: off
- `--view-only`: View with Glow, default: off
- `-q`, `--quiet`: Minimal output, default: quiet on
- `-v`, `--verbose`: Verbose output, default: verbose off

### st-fact

Source: `cross_st/st-fact.py`.

- `json_file`: Path to the JSON file
- `--all`: Fact check all stories in container
- `--agent`: AI to use, or "all" to run all AIs in parallel (default: [configured value])
- `--cache`: Enable API cache, default: enabled
- `--no-cache`: Disable API cache
- `-s`, `--story`: Fact check a single story (integer), default 1
- `--file`: Write to a file, default is no-file
- `--paragraph`: Write paragraphs a _paragraph_test_n.txt file, default is no
- `--display`: Write results to the display, default is yes-display
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output
- `--silent`: Suppress all output including tqdm progress bars; used when called from st-cross as a child process.
- `--timeout`: Timeout value: with --agent all, sets per-job limit in minutes (default: 20). In single-AI mode, sets per-paragraph limit in seconds (default: 0 = no timeout).
- `--retry-budget`: Total retry budget in seconds passed through to process_prompt() (default: 0 = unlimited). When set, transient errors are retried only until the budget is exhausted; useful for parallel runs (st-cross --parallel) where a single 105 s tail would stall the matrix. Independent of --timeout: --retry-budget is per API call (per segment), --timeout is per subprocess.

### st-fetch

Source: `cross_st/st-fetch.py`.

- `--file`: Import a plain text, markdown, or PDF file from disk
- `--url`: Fetch a web page and extract its text
- `--clipboard`: Import text from the system clipboard
- `tweet_id`: Tweet ID to fetch from X (numeric ID from post URL)
- `json_file`: Path to the .json container
- `--cache`: Enable API cache (default: on)
- `--no-cache`: Disable API cache — always fetch live
- `--prep`: Run st-prep after fetching (default: on)
- `--no-prep`: Skip st-prep — store as raw data entry only
...
Source: `cross_st/st-find.py`.

- `pattern`: Search pattern - multiple keywords supported (use quotes for wildcards)
- `-t`, `--title`: Search in story titles
- `-p`, `--prompt`: Search in prompts
- `-s`, `--story`: Search in story text/markdown/spoken
- `-a`, `--all`: Search in all fields (default if no flags specified)
- `-r`, `--recursive`: Search recursively in subdirectories
- `-v`, `--verbose`: Enable verbose output

### st-fix

Source: `cross_st/st-fix.py`.

- `-s`, `--story`: Story number to fix (default: auto-select most fixable)
- `-f`, `--fact`: Fact-check number to use (default: auto-select most fixable). Not used in synthesize mode.
- `--mode`: Fix strategy (default: iterate)
- `json_file`: Path to the JSON file
- `--agent`: AI to use for rewriting (default: same AI that wrote the story)
- `--checker`: AI to use for inline fact-checking in iterate mode (default: the original fact-checker from -f)
- `--cache`: Enable API cache (default: on)
- `--no-cache`: Disable API cache
- `--prep`: Run st-prep on the result
- `--dry-run`: Show what would be changed without calling any AI or writing to the file
- `-v`, `--verbose`: Show diff of each change
- `-q`, `--quiet`: Minimal output
- `--include-fixed`: Include reports previously generated by st-fix in auto-selection. Default: skip them so the originals are fixed instead of "fixing the fix".

### st-gen

Source: `cross_st/st-gen.py`.

- `prompt`: Path to the prompt file
- `--agent`: Define AI model to use, default is [configured value]
- `--cache`: Enable API cache, default: enabled
- `--no-cache`: Disable API cache
- `--no-prep`: Skip st-prep after generation (default: st-prep runs automatically)
- `--bang`: Parallel AI execution, 0 is first thread, default -1 is serial execution
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output
- `--ai-title`: Generate a title (max 10 words) → stdout
- `--ai-short`: Generate a short summary (max 80 words) → stdout
...
Source: `cross_st/st-heatmap.py`.

- `json_file`: Path to the JSON file
- `--display`: Display heatmap on screen, default: off
- `--file`: Save heatmap PNG to file, default: off
- `--path`: Output directory for PNG file, default: ./tmp
- `--ai-title`: Generate a title for the heatmap (max 10 words) → stdout
- `--ai-short`: Generate a short caption (max 80 words) → stdout
- `--ai-caption`: Generate a detailed caption (100–160 words) → stdout
- `--ai-summary`: Generate a concise summary (120–200 words) → stdout
- `--ai-story`: Generate a comprehensive story (800–1200 words) → stdout
- `--agent`: AI to use for content generation (default: [configured value])
- `--cache`: Enable API cache, default: enabled
- `--no-cache`: Disable API cache
- `-v`, `--verbose`: Verbose output
- `-q`, `--quiet`: Minimal output
...
Source: `cross_st/st-ls.py`.

- `json_file`: Path to the JSON file
- `-a`, `--all`: Display all sections: data, stories, fact-checks, and claims
- `-d`, `--data`: Display container data
- `-s`, `--story`: Display container stories
- `-f`, `--fact`: Display container fact-check data
- `-C`, `--claims`: Display cross-AI claims comparison table (seg x AI verdict)
- `-c`, `--clip`: Clip title length (integer), default 60
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output
- `--score-weights`: Override composite-score weights, e.g. "cov=0.25,comp=0.25,acc=0.40,cal=0.10". Keys: cov, comp, acc, cal (or full names).
...
Source: `cross_st/st-man.py`.

- `command`: Command name, or faq; omit to list commands.
- `--web`: Open the corresponding wiki page in a browser.
- `--doc`: Print the source docstring instead of the formatted man page.

### st-merge

Source: `cross_st/st-merge.py`.

- `json_file`: Path to the JSON file
- `--agent`: Synthesizer AI for simple mode. In quality mode the base story author is always used. Default: [configured value]
- `--cache`: Enable API cache, default: enabled
- `--no-cache`: Disable API cache
- `-a`, `--all`: Merge all stories, default is no, use stories
- `--stories`: Merge selected stories (default: all original stories, excluding fix/merge derived stories)
- `--base`: Story number to use as structural base (default: auto, highest score)
- `--min-score`: Exclude stories with avg fact-check score below this threshold (e.g. --min-score 1.2). Applied before auto-selection.
- `--simple`: Force simple merge — ignore fact-check data
- `--quality`: Force quality merge — fail if no fact-check data exists
- `--post-check`: Run st-fact on merged story and show before/after score (default: on)
- `--no-post-check`: Skip post-merge fact-check
- `-v`, `--verbose`: Enable verbose output
- `-q`, `--quiet`: Enable minimal output
- `--ai-title`: Generate a title (max 10 words) → stdout
...
Source: `cross_st/st-new.py`.

- `prompt`: Name for the prompt file, with or without .prompt
- `-t`, `--template`: Template to use
- `-g`, `--gen`: Run st-gen (+ st-prep) automatically after editing
- `--agent`: AI provider for --gen (default: [configured value])
- `-b`, `--bang`: Start "st-bang" app after editing, render with all AI. Start "st" if you also use "--st".
- `-s`, `--st`: Start the "st" app after editing, render with default AI
- `-n`, `--no-spell`: Disable spell check
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output

### st-plot

Source: `cross_st/st-plot.py`.

- `json_file`: Path to the JSON file
- `--plot`: Consult command help for details.
- `--file`: Write plots to a file, default is no-file
- `--path`: Path for output files, default is ./tmp directory in current location
- `--file_kv`: Print json file_kv with play:url pairs, default is no-print
- `--display`: Display plots on screen, default is no-display
- `--json`: Save data to json file, default is no-file
- `--csv`: Save data to csv file, default is no-file
- `--markdown`: Save data to markdown file, default is no-file
- `--no-cache`: Bypass the API cache for AI content generation
- `--cache`: Use the API cache for AI content generation (default)
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output
- `--agent`: AI provider for content generation (default: uses DEFAULT_AI)
- `--ai-title`: Generate a short title (max 10 words) → stdout
- `--ai-short`: Generate a short paragraph (max 80 words) → stdout
- `--ai-caption`: Generate a detailed caption (100–160 words) → stdout
- `--ai-summary`: Generate a concise summary (120–200 words) → stdout
- `--ai-story`: Generate a comprehensive story (800–1200 words) → stdout
...
Source: `cross_st/st-post.py`.

- `files`: Files of to process
- `--site`: Select Discourse site to use, default is [configured value]
- `-s`, `--story`: Select story to publish: default 1
- `-f`, `--fact`: Reply with fact-check to the post, default: no reply
- `--category`: Where to post. Accepts: "private" (your private area), a numeric Discourse category ID, or — on crossai.dev only — "test" (id=6, sandbox cleared daily), "reports" (id=16, public portfolio), "prompt-lab" (id=17). Default: "test".
- `--check`: Validate Discourse credentials and connection without posting
- `--prompt`: Post the prompt text from the container instead of the story (useful with --category prompt-lab)
- `-v`, `--verbose`: Verbose output, default: verbose off
- `-q`, `--quiet`: Enable minimal output

### st-prep

Source: `cross_st/st-prep.py`.

- `json_file`: Path to the JSON file
- `-d`, `--data`: Data entry to process (integer index), default: 1
- `-a`, `--all`: Export all formats: md, mp3, title, txt, default: off
- `-md`, `--markdown`: Export story as .md file, default: off
- `-mp3`, `--mp3`: Render and export story as .mp3 audio, default: off
- `-title`, `--title`: Export story title as .title file, default: off
- `-txt`, `--txt`: Export story as .txt file, default: off
- `--bang`: Parallel execution thread index (0 = first thread), default: -1 (serial)
- `-v`, `--verbose`: Verbose output, default: off
- `-q`, `--quiet`: Minimal output, default: off

### st-print

Source: `cross_st/st-print.py`.

- `json_file`: Path to the story container JSON file
- `-s`, `--story`: Story number to print (1-based, default: 1)
- `--all`: Print / save all stories in the container
- `--save-pdf`: Save PDF to a file instead of printing (auto-names from container)
- `--output`: Explicit output PDF filename (implies --save-pdf)
- `--preview`: Save PDF and open it in the system PDF viewer
- `--printer`: Send to a named printer (passed to lpr -P); default printer if omitted
- `-v`, `--verbose`: Verbose output
- `-q`, `--quiet`: Suppress informational output

### st-read

Source: `cross_st/st-read.py`.
...
Source: `cross_st/st-rm.py`.

- `json_file`: Path to the JSON file
- `-d`, `--data`: Remove a data item (integer index)
- `-s`, `--story`: Remove a story (integer index)
- `-f`, `--fact`: Remove a fact-check report from a story (integer index)
- `-F`, `--clear-facts`: Clear all fact-checks (and segments) from a story (-s) or all stories
- `--all-stories`: Apply --clear-facts to every story in the container
- `-v`, `--verbose`: Enable verbose output
- `-q`, `--quiet`: Enable minimal output
...
Source: `cross_st/st-speak.py`.

- `json_file`: Path to the JSON file
- `-s`, `--story`: Story to convert (integer), default 1
- `--source`: Select text source. Options: 'text' (default) or 'fact'.
...
Source: `cross_st/st-speed.py`.

- `json_file`: Path to JSON file(s)
- `--agent`: AI to use for content generation (default: auto). When used with --ai-* flags, selects which AI generates the content but does NOT filter the performance display. Without --ai-* flags, also filters the performance display to one provider.
- `--ai-title`: Generate short title (max 10 words) → stdout
- `--ai-short`: Generate short caption (max 80 words) → stdout
- `--ai-caption`: Generate detailed caption (100-160 words) → stdout
- `--ai-summary`: Generate concise summary (120-200 words) → stdout
- `--ai-story`: Generate comprehensive story (800-1200 words) → new story in JSON
- `--csv`: Export raw data to CSV file
- `--history`: Analyze trends across multiple files
- `--cache`: Enable API response caching (default for AI content generation)
- `--no-cache`: Disable API response caching (forces fresh AI calls)
- `--quiet`, `-q`: Minimal output
- `--verbose`, `-v`: Verbose output (show generation details)

### st-stones

Source: `cross_st/st-stones.py`.

- `paths`: Directory containing benchmark .json/.prompt pairs, or explicit .json file paths.  If omitted, ~/cross-stones/ or ./cross_stones/ is used automatically.
- `--init`: Seed the benchmark domain directory with bundled .prompt files and exit.  Default destination: ~/cross-stones/.
- `--dir`: Destination directory for --init  (default: ~/cross-stones/)
- `--run`: Run st-cross for any domain that lacks fact-check data
- `--confirmation`: Prompt for confirmation before making API calls with --run (default)
- `--no-confirmation`: Skip the --run confirmation prompt (useful for scripting)
- `--w1`: Accuracy weight 0..1  (default [configured value])
- `--w2`: Speed weight 0..1  (default [configured value])
- `--no-speed`: Ignore timing data; pure accuracy scoring (sets w1=1.0, w2=0.0)
- `--n-claims`: Expected scorable claims per domain prompt  (default: value from benchmark set config, or [configured value])
- `--domain`: Show per-domain fact-score breakdown table
- `--cache`: Enable API response cache for --run  (default: enabled)
- `--no-cache`: Consult command help for details.
- `--timeout`: Per-job timeout in seconds passed to st-cross --run  (default: 300 = 5 min).  0 = no timeout.
- `-v`, `--verbose`: Consult command help for details.
- `-q`, `--quiet`: Consult command help for details.
- `--score-weights`: Override composite-score weights, e.g. 'cov=0.25,comp=0.25,acc=0.40,cal=0.10'. Keys: cov, comp, acc, cal (or full names).
- `--history`: Display the full snapshot history stored in the benchmark set config (composite score, speed ratio, and accuracy over time)
- `--set-baseline`: Lock the current run's average timing as the speed baseline in the benchmark set config.  Run once after the first complete benchmark. Future runs scoring faster will have speed_ratio > 1.0 and cross_stone_score may exceed 1.0, showing genuine improvement.
- `--record-snapshot`: Append the current leaderboard scores as a named snapshot in the benchmark set config, building a year-over-year history.
- `--snapshot-label`: Human-readable label for --record-snapshot  (default: today's ISO date, e.g. '2026-03-27')
- `--ai-title`: Generate a ≤10-word leaderboard title → stdout
- `--ai-short`: Generate a 40-80-word leaderboard summary → stdout
- `--ai-caption`: Generate a 100-160-word two-paragraph caption → stdout
- `--ai-summary`: Generate a concise summary (120–200 words) → stdout
- `--ai-story`: Generate a comprehensive story (800–1200 words) → stdout
- `--agent`: Agent to use for content generation  (default: [configured value])

### st-verdict

Source: `cross_st/st-verdict.py`.
...
- `--path`: Output directory for PNG file, default: ./tmp
- `--ai-title`: Generate a title (max 10 words) → stdout
- `--ai-short`: Generate a short caption (max 80 words) → stdout  [default: on when no other --ai-* flag is given]
- `--no-ai-short`: Suppress the automatic short caption
- `--ai-caption`: Generate a detailed caption (100–160 words) → stdout
- `--ai-summary`: Generate a concise summary (120–200 words) → stdout
- `--ai-story`: Generate a comprehensive story (800–1200 words) → stdout
- `--agent`: AI to use for content generation (default: [configured value])
- `--what-is-false`: Aggregate all claims marked false / partially_false across fact-checkers and ask the AI for a focused breakdown
- `--what-is-true`: Aggregate all claims marked true / partially_true and ask the AI for a focused supporting summary
- `--what-is-missing`: Identify what important aspects of the prompt the report failed to mention (omissions / gaps)
- `--how-to-fix`: Recommend exactly one next action: st-fix, st-bang -N, st-merge, or publish-as-is (never auto-invokes)
- `-s`, `--story`: Story index to analyse with the lens (default: winning story per score_authors(); falls back to 1 if no winner can be resolved)
- `--cache`: Enable API cache, default: enabled
- `--no-cache`: Disable API cache
- `-v`, `--verbose`: Verbose output
- `-q`, `--quiet`: Minimal output
- `--no-render`: Print raw markdown for AI content instead of rendered (styled) output
- `--score-weights`: Override composite-score weights, e.g. "cov=0.25,comp=0.25,acc=0.40,cal=0.10". Keys: cov, comp, acc, cal (or full names).

### st-voice

Source: `cross_st/st-voice.py`.

- `text_file`: Path to a text file (optional)
- `-voices`, `--voices`: Print names of all onyx voices, default: off
- `-curl`, `--curl`: Print curl commands to download all onyx voices, default: off
- `-k`, `--kill`: Kill gimp server on port 6419, default is do not kill
- `-v`, `--verbose`: Enable verbose output, default is verbose
- `-q`, `--quiet`: Enable minimal output

### st-ask

Source: `cross_st/st-ask.py`.

- `question`: Ask a question
- `--agent`: Agent name to use for LLM tier
- `--pseudo`: Force Pseudo-AI tier even if API key present
- `--explain-last-error`: Explain the last error using the LLM tier
- `--no-render`: Print raw markdown instead of rendered (styled) output
- `--no-feedback`: Skip the post-answer thumb-up/down feedback prompt
