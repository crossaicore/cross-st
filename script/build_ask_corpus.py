...
Usage:
  ...
  python script/build_ask_corpus.py --check-current  # validate freshness + command pages
...
from audit_ask_commands import command_names, command_reference
...
def assemble_corpus(entries: list[dict], version: str) -> str:
    ...
    line 1, then labelled reference sections.
    """
    out: list[str] = []
    ...
    out.append(
        ...
        "`cross_st/data/support_faq.md`, and CLI argument declarations) and re-run the build script."
    )
    out.append("")
    ...
    # Current parser declarations provide a reproducible reference for all
    # entry points, including flags missing from the prose wiki pages.
    out.extend(["---", "", command_reference(), ""])
...
def main() -> int:
    ...
    p.add_argument(
        "--check-current",
        action="store_true",
        help="Validate FAQ, command pages, and exact corpus freshness without writing.",
    )
    p.add_argument(
        "--version",
        action="store_true",
    ...
    if args.check_current:
        missing = [name for name in command_names() if not (WIKI_DIR / f"{name}.md").is_file()]
        if missing:
            print(f"Missing command wiki pages: {', '.join(missing)}", file=sys.stderr)
            return 1
        if not OUT_PATH.exists() or OUT_PATH.read_text(encoding="utf-8") != corpus:
            print("Corpus is stale; run python script/build_ask_corpus.py", file=sys.stderr)
            return 1
        print("  FAQ, command pages and corpus freshness checks passed")
        return 0
